...
server.tool("create_bay", "Create an isolated git worktree (bay) for a pit call", {
...
server.tool("list_bays", "List all active git worktree bays", {
    repo_path: z.string().describe("Absolute path to the git repository"),
}, async ({ repo_path }) => {
    const output = run("git worktree list --porcelain", repo_path);
    return { content: [{ type: "text", text: output }] };
});
server.tool("release_bay", "Merge a bay back to the current branch and clean up", {
    repo_path: z.string().describe("Absolute path to the git repository"),
...
server.tool("cleanup_bays", "Remove all pit worktrees and branches", {
    repo_path: z.string().describe("Absolute path to the git repository"),
}, async ({ repo_path }) => {
...
server.tool("create_pit_call", "Create a new pit call (bead) for tracking", {
    ...
    const output = run(`${BD} create "${title}" -d "${description.replace(/"/g, '\\"')}" -t task ${prio} ${labels}`);
    return { content: [{ type: "text", text: output }] };
});
server.tool("list_pit_calls", "List all pit calls with their status", {}, async () => {
    const output = run(`${BD} list`);
    return { content: [{ type: "text", text: output }] };
});
server.tool("claim_pit_call", "Claim a pit call (set to in_progress)", {
    bead_id: z.string().describe("Bead ID to claim"),
}, async ({ bead_id }) => {
    const output = run(`${BD} update ${bead_id} --claim`);
    return { content: [{ type: "text", text: output }] };
});
server.tool("close_pit_call", "Close a completed pit call", {
...
server.tool("timing_screen", "Show pit stop status dashboard", {
    repo_path: z.string().describe("Absolute path to the git repository"),
}, async ({ repo_path }) => {
...
server.tool("load_context", "Load .pitcrew and .pitcrew-lessons for mechanic prompt injection", {
    repo_path: z.string().describe("Absolute path to the git repository"),
}, async ({ repo_path }) => {
    const parts = [];
...
server.tool("add_lesson", "Add a lesson learned from a pit stop to .pitcrew-lessons", {
    repo_path: z.string().describe("Absolute path to the git repository"),
